...
{{< hint warning >}}
__Limitations:__
- The table must be created with **`'branch-merge.enabled' = 'true'`**. This option restricts the table to pure-append commits only (no compaction, no INSERT OVERWRITE, no deletion vectors), which guarantees merge correctness.
