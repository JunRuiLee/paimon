...
class MergeBranchProcedureTest extends PaimonSparkTestBase {
  ...
  test("Paimon procedure: merge branch with explicit target branch") {
    ...
    spark.sql(
      "CALL paimon.sys.create_branch(table => 'test.T', branch => 'branch_a', tag => 'tag1')")
    spark.sql(
      "CALL paimon.sys.create_branch(table => 'test.T', branch => 'branch_b', tag => 'tag1')")

    spark.sql(s"INSERT INTO `T$$branch_branch_a` VALUES (2, 'b')")

    checkAnswer(
      spark.sql(
        "CALL paimon.sys.merge_branch(table => 'test.T', source_branch => 'branch_a', target_branch => 'branch_b')"),
      Row(true) :: Nil)

    checkAnswer(
      spark.sql("SELECT * FROM `T$$branch_branch_b` ORDER BY id"),
      Row(1, "a") :: Row(2, "b") :: Nil)
  }
