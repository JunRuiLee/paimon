...
public class FileSystemBranchManager implements BranchManager {
    ...
    public void mergeBranch(String sourceBranch, String targetBranch) {
        ...
        validateBranchMergeEnabled(sourceBranch);
    ...
    private void validateBranchMergeEnabled(String sourceBranch) {
        SchemaManager sm = new SchemaManager(fileIO, tablePath, sourceBranch);
        TableSchema schema = sm.latest().get();
        CoreOptions opts = new CoreOptions(schema.options());
        checkArgument(
                opts.branchMergeEnabled(),
                "Branch merge requires '%s' to be true.",
        ...
        for (ManifestEntry entry : files.values()) {
            checkArgument(
                    ...
                    "Cannot merge branch '%s', because it contains compacted files (level > 0). "
                            + "Please ensure no compaction has been performed on either branch before merging.",
                    branchName);
        }
    }
    ...
    private List<ManifestEntry> computeMergeDiff(String sourceBranch, String targetBranch) {
        Set<FileEntry.Identifier> targetFileIds =
                mergeHandler.readBranchFileIdentifiers(targetBranch);
